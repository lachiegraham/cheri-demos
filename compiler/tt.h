...
typedef struct _decl_ast_s {
    ast_object;
    char *value;
    char *valuetype;
} decl_ast_t;

#define new_decl_ast(x, v, t, no) \
    x = (decl_ast_t*)malloc(sizeof(decl_ast_t)); \
    x->line = no; \
    x->type = DECLAST; \
    x->value = v; \
    x->valuetype = t
...
typedef struct _const_ast_s {
    ast_object;
    char *value;
    char *valuetype;
} const_ast_t;

#define new_const_ast(x, v, t, no) \
    x = (const_ast_t*)malloc(sizeof(const_ast_t)); \
    x->line = no; \
    x->type = CONSTAST; \
    x->value = v; \
    x->valuetype = t
...
void translate_primary(expressions*, FILE*);
